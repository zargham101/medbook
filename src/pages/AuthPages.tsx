...
import { Mail, Lock, User, ArrowRight, AlertCircle, Stethoscope, UserRound } from 'lucide-react';
    ...
    <div className="flex min-h-screen">
      ...
      <div className="relative hidden lg:block lg:flex-1">
        ...
        <div className="relative flex h-full flex-col justify-center p-20 text-white">
          <h2 className="text-3xl font-bold leading-tight">Your health, on your schedule.</h2>
          ...
          <div className="mt-10 space-y-4">
            {[
              ...
              <div key={f.text} className="flex items-center gap-3 text-teal-50">
                <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/15">{f.icon}</span>
                {f.text}
              </div>
            ))}
          </div>
...
export function LoginPage() {
  ...
  return (
    <AuthShell
      ...
      footer={<>Don&apos;t have an account? <Link to="/signup" className="font-semibold text-teal-600 hover:text-teal-700">Create one</Link></>}
    >
      {error && (
        <div className="mb-4 flex items-center gap-2 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700 ring-1 ring-inset ring-red-200">
      ...
      <div className="mt-6 rounded-lg bg-slate-50 px-4 py-3 text-xs text-slate-500 ring-1 ring-inset ring-slate-200">
        <p className="font-medium text-slate-600">Try a demo doctor account:</p>
...
export function SignupPage() {
  ...
  if (success) {
    return (
      <AuthShell
        ...
        footer={<Link to="/login" className="font-semibold text-teal-600 hover:text-teal-700">Back to sign in</Link>}
      >
        <div className="rounded-2xl bg-teal-50 p-6 text-center ring-1 ring-inset ring-teal-200">
  ...
  return (
    <AuthShell
      ...
      footer={<>Already have an account? <Link to="/login" className="font-semibold text-teal-600 hover:text-teal-700">Sign in</Link></>}
    >
      {error && (
        <div className="mb-4 flex items-center gap-2 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700 ring-1 ring-inset ring-red-200">
          <AlertCircle className="h-4 w-4 shrink-0" /> {error}
  ...
  return (
    <button
      ...
      className={`flex items-center gap-3 rounded-xl border p-3 text-left transition-all ${
        active ? 'border-teal-600 bg-teal-50 ring-1 ring-teal-600' : 'border-slate-300 bg-white hover:border-slate-400'
      }`}
    >
      <span className={`flex h-9 w-9 items-center justify-center rounded-lg ${active ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{icon}</span>
      <div>
        <p className={`text-sm font-semibold ${active ? 'text-teal-700' : 'text-slate-900'}`}>{label}</p>
